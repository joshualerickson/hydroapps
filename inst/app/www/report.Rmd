...
```{r, echo=FALSE}
final_cul()$together_long %>% dplyr::filter(RI %in% c(50,100)) %>% dplyr::select(`Return Interval` = ReturnInterval,Source = source, Method, Value = value, Size) %>% kableExtra::kable() %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = F)
```
