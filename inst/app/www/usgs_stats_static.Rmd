...
if(dv_report[1,]$Station %in% 'Tobacco River near Eureka MT'){
  ...
  wildlandhydRo::plot_USGSdrought(nwis_sites_df,
                                rolln = 1, 
                                startDate = max(nwis_sites_df$Date)- 365,
                                endDate = max(nwis_sites_df$Date)) +
...
```{r,fig.align='center', echo=F,error=F, message=F, results='hide'}
month_abb <- data.frame(month = seq(1,12,1), month_abb = month.abb)
nwis_sites_df %>% wildlandhydRo::wyUSGS() %>% 
