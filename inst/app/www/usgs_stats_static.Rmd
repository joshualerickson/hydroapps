...
```{r, echo=FALSE, warning=FALSE,error=F, message = F, fig.align='center', fig.width=11, fig.height=6}
if(dv_report[1,]$Station %in% 'Tobacco River near Eureka MT'){
  
  dv_report2 <- dv_report %>% mutate(current_daily_mean_flow = Flow) %>% 
    filter(Date <= max(Date), Date >= max(Date)-365)
g1 <- plot_reportUSGS_custom(dv_report2) +
  ...
  g1 <- wildlandhydRo::plot_reportUSGS(dv_report) +
    ggplot2::theme(axis.text.x = ggplot2::element_text(size = 10),
        axis.text.y = ggplot2::element_text(size = 10)) + 
    ggplot2::scale_y_continuous()
  plotly::ggplotly(g1)
...
```{r,fig.align='center', echo=F,error=F, message=F}
