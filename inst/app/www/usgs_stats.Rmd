---
title: "Discharge Report for USGS Gauging Station: <br> <span style='color: black; font-size: 30px'> `r paste0(usgs_ggplot_data_not_filtered() %>% dplyr::slice(n=1) %>% .$Station)`</span>"
date: "`r Sys.Date()`"
output: 
  html_document:
...
one_row <- usgs_ggplot_data_not_filtered() %>% slice(n=1)
...
if(one_row$Station %in% 'Tobacco River near Eureka MT'){
  ...
  summary_stats <- usgs_ggplot_data_not_filtered()  %>% 
  ...
  right_join(usgs_ggplot_data_not_filtered(), by = c("Station", "month_abb")) %>% 
  mutate(across(contains("_va"), round, 1)) %>% 
  ungroup() %>% 
  group_by(month_abb) %>% slice(n=1) %>% ungroup()%>% 
  mutate(across(p05_va:p95_va, round, 0))
  
   max_flow <- usgs_ggplot_data_not_filtered() %>% 
  filter(month == month_station, day == day_station) %>% 
  slice_max(Flow) %>% slice(n = 1)
  
  max_dates <- usgs_ggplot_data_not_filtered() %>% 
  filter(month == month_station, day == day_station) %>% 
    slice_max(Flow) %>% 
    pull(year)
  
  min_flow <- usgs_ggplot_data_not_filtered() %>% 
  filter(month == month_station, day == day_station) %>% 
  slice_min(Flow) %>% slice(n = 1)
  
  min_dates <- usgs_ggplot_data_not_filtered() %>% 
  filter(month == month_station, day == day_station) %>% 
    slice_min(Flow) %>% 
    pull(year)
...
dv_report <- usgs_ggplot_data_not_filtered() %>% 
  ...
  summarise(quantiles = quantile(Flow, probs = c(0.05, 0.1, 0.2, 0.25, 0.5,
            ...
            breaks =  c("p05_va", "p10_va", "p20_va", "p25_va", "p50_va", "p75_va",
                        "p80_va", "p90_va", "p95_va")) %>% 
  pivot_wider(values_from = quantiles, names_from = breaks)  %>% 
  right_join(usgs_ggplot_data_not_filtered(), by = c("Station", "month_day")) %>% 
  ...
  hr_flow <- usgs_ggplot_data_not_filtered() %>% wildlandhydRo::hourlyUSGS(days = 30) 
...
dv_report <- wildlandhydRo::reportUSGSdv(usgs_ggplot_data_not_filtered(), days = 365) %>% dplyr::filter(!is.na(Date))
...
last_flow <- joined_flows %>% group_by(name) %>% 
  ...
  dv_today <- dv_today %>% mutate(current_monthly_mean_flow = usgs_ggplot_data_not_filtered() %>% 
...
`r paste0("<b>Station Name: </b>", paste(one_row$Station),           "<br>","<b>Drainage Area: </b>", paste(one_row$drainage_area, " sq.mi"),           "<br>","<b>Site #: </b>",paste(one_row$site_no), "<br>", "<b>Current Flow (instantaneous): </b>", paste(round(cur_flow$value, 2), " <sup>ft<sup>3</sup></sup>&frasl;<sub>s</sub>"), "<br>", "<b>Current Flow (mean): </b>", paste(round(dv_today$current_daily_mean_flow, 2), " <sup>ft<sup>3</sup></sup>&frasl;<sub>s</sub>"),"<br>", "<b>Current Monthly Flow (mean): </b>", paste(round(dv_today$current_monthly_mean_flow, 2), " <sup>ft<sup>3</sup></sup>&frasl;<sub>s</sub>"), "<br>", "<b>Minimum Flow: </b>", paste(round(dv_today$min_va,2), " <sup>ft<sup>3</sup></sup>&frasl;<sub>s</sub>", ' (', dv_today$min_va_yr, ')'), "<br>", "<b>Maximum Flow: </b>", paste(round(dv_today$max_va,2), " <sup>ft<sup>3</sup></sup>&frasl;<sub>s</sub>", ' (', dv_today$max_va_yr, ')'), "<br>", "<b>Period of Record: </b>", paste(min(usgs_ggplot_data_not_filtered()$Date), ' to ', max(usgs_ggplot_data_not_filtered()$Date)),
"<br>", "<b>Years with Data < 355 days: </b>", paste(' ', usgs_ggplot_data_not_filtered() %>% dplyr::count(year) %>% dplyr::filter(n < 355) %>% tidyr::pivot_wider(names_from = n, values_from = year, names_glue = "count_{n}") %>% dplyr::mutate(years_missing = paste(unlist(.), collapse = ', ')) %>% dplyr::pull(years_missing)))`
:::
...
dv_today_gt %>%
  gt::tab_header(
    title = gt::md("**Percentiles:**"),
    subtitle = gt::md("**Daily Stats**")
  ) %>% gt::sub_missing(columns = vars(-`Month-Day`)) %>% 
  gt::tab_style(
    style = list(
      gt::cell_fill(color = "#FF0000", alpha = 0.5)
      ),
...
mv_stats_gt %>%
  ...
  ) %>% gt::sub_missing(columns = vars(-Month)) %>% 
  gt::tab_style(
    style = list(
      gt::cell_fill(color = "#FF0000", alpha = 0.5)
      ),
...
if(!is.null(hr_flow)){
g<-ggplot(data = joined_flows, aes(Date, value, color = name)) + 
...
<center>
```{r, echo=FALSE,warning=F, error=F, message=F, fig.width=11}
...
peak_count <- usgs_ggplot_data_not_filtered() %>% wildlandhydRo::wyUSGS() %>% 
...
<!-- basin_sf <- usgs_ggplot_data_not_filtered() %>% -->
